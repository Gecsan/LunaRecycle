...
ENABLE_MOTORS = True
# Default to CTRE CANivore naming and allow override from environment.
# Example overrides:
#   CAN_BUS_NAME=CanBus  (typical CANivore name)
#   CAN_BUS_NAME=can0    (Linux socketcan interface)
CAN_BUS_NAME = os.getenv("CAN_BUS_NAME", "CanBus")
...
# Estimated torque conversion used for display: Nm = torque_current(A) * constant.
# Override with MOTOR_TORQUE_NM_PER_AMP if you have a calibrated value.
MOTOR_TORQUE_NM_PER_AMP = float(os.getenv("MOTOR_TORQUE_NM_PER_AMP", "0.019"))
...
def make_can_bus():
    ...
    # TalonFX accepts a bus-name string and this works for both socketcan and CANivore.
    return CAN_BUS_NAME
...
def refresh_motor_detected(entry):
    try:
        ensure_motor_object(entry)

        version_signal = entry.get("version_signal")
        if version_signal is None:
            version_signal = entry["motor"].get_version()
            entry["version_signal"] = version_signal

        if hasattr(version_signal, "refresh"):
            version_signal.refresh()

        status = getattr(version_signal, "status", None)
        if status is None:
            entry["detected"] = False
            entry["detection_status"] = "UNKNOWN"
            return False

        entry["detection_status"] = str(status)
        # Phoenix can report is_all_good=True even when status is RX_TIMEOUT.
        # Treat only explicit OK as detected.
        status_name = getattr(status, "name", "")
        detected = status_name == "OK"
        ...
        entry["detected"] = False
        entry["detection_status"] = f"ERROR: {e}"
        entry["last_error"] = str(e)
        return False
...
class RecyclingSystemGUI:
    def toggle_motor_enable(self, device_id):
        # Toggle the enable state and update button color/text
        current = self.motor_enabled_vars[device_id].get()
        new_state = not current
        self.motor_enabled_vars[device_id].set(new_state)
        self.motor_switch_text_vars[device_id].set("ON" if new_state else "OFF")
        self.update_motor_enable_buttons()
        self.on_motor_switch_changed(device_id)

    def update_motor_enable_buttons(self):
        # Update button color based on enable state
        for device_id, btn in getattr(self, 'motor_enable_buttons', {}).items():
            enabled = self.motor_enabled_vars[device_id].get()
            if enabled:
                btn.configure(bg="#27ae60", activebackground="#27ae60")  # Green
            else:
                btn.configure(bg="#c0392b", activebackground="#c0392b")  # Red
    def __init__(self, root):
        ...
        self.main_frame = ttk.Frame(self.canvas, padding=12)
    ...
    def build_servo_section(self):
        servo_frame = ttk.LabelFrame(self.main_frame, text="MG90S Servo Control", padding=8)
        servo_frame.pack(fill="x", expand=False, pady=(0, 12))

        ttk.Label(servo_frame, text="Angle (0-180)").grid(row=0, column=0, padx=6, pady=4)
        self.servo_angle_var = tk.StringVar(value="90")
        angle_entry = ttk.Entry(servo_frame, textvariable=self.servo_angle_var, width=8)
        angle_entry.grid(row=0, column=1, padx=6, pady=4)

        self.servo_step_var = tk.StringVar(value="5")
        ttk.Label(servo_frame, text="Step").grid(row=0, column=2, padx=6, pady=4)
        ttk.Entry(servo_frame, textvariable=self.servo_step_var, width=6).grid(row=0, column=3, padx=6, pady=4)

        ttk.Button(
            servo_frame,
            text="<",
            width=4,
            command=lambda: self.on_servo_nudge(-1),
        ...
        self.servo_status_var = tk.StringVar(value="Idle")
        ttk.Label(servo_frame, textvariable=self.servo_status_var).grid(row=0, column=7, padx=6, pady=4)

        rotate_btn = ttk.Button(servo_frame, text="Rotate", command=self.on_servo_rotate)
        rotate_btn.grid(row=0, column=6, padx=6, pady=4)

    def _read_servo_angle(self):
        angle_str = self.servo_angle_var.get().strip()
    ...
    def _read_servo_step(self):
        step_str = self.servo_step_var.get().strip()
    ...
    def _command_servo_angle(self, angle):
        import subprocess

        try:
            result = subprocess.run([
                "python3", "servo_control.py", str(angle)
            ], capture_output=True, text=True, cwd=".")
            if result.returncode == 0:
                self.servo_status_var.set(f"Rotated to {angle:.0f}°")
            else:
                self.servo_status_var.set(f"Error: {result.stderr.strip()}")
        except Exception as e:
            self.servo_status_var.set(f"Failed: {e}")

    def on_servo_rotate(self):
        try:
            angle = self._read_servo_angle()
        except Exception:
            self.servo_status_var.set("Invalid angle")
            return

        try:
            self._command_servo_angle(angle)
        except Exception as e:
            self.servo_status_var.set(f"Failed: {e}")

    def on_servo_nudge(self, direction):
        try:
            angle = self._read_servo_angle()
            step = self._read_servo_step()
        except ValueError as e:
            self.servo_status_var.set(str(e))
            return

        new_angle = max(SERVO_MIN_ANGLE, min(SERVO_MAX_ANGLE, angle + (step * direction)))
        self.servo_angle_var.set(f"{new_angle:.0f}")
        self._command_servo_angle(new_angle)
    ...
    def build_motor_table(self):
        ...
        for row, cfg in enumerate(MOTOR_CONFIGS, start=1):
            ...
            # Store the button widget for color updates
            if not hasattr(self, 'motor_enable_buttons'):
                self.motor_enable_buttons = {}
            rpm_var = tk.StringVar(value="0")
            detected_var = tk.StringVar(value="NO")
            ...
            self.motor_detected_vars[device_id] = detected_var
            ...
            if not hasattr(self, 'motor_torque_vars'):
                self.motor_torque_vars = {}
            self.motor_torque_vars[device_id] = torque_var
            ...
            enable_btn = tk.Button(
                ...
                width=8,
                height=2,
                command=lambda motor_id=device_id: self.toggle_motor_enable(motor_id),
                bg="#c0392b",  # Red by default
                fg="white",
                activebackground="#27ae60",  # Green when pressed
                font=("Arial", 12, "bold")
            )
            enable_btn.grid(row=row, column=1, sticky="w", padx=6, pady=4)
            self.motor_enable_buttons[device_id] = enable_btn
            ttk.Label(self.motor_frame, text=str(device_id)).grid(row=row, column=2, sticky="w", padx=6, pady=4)
            ttk.Label(self.motor_frame, textvariable=detected_var).grid(row=row, column=3, sticky="w", padx=6, pady=4)
            ttk.Entry(self.motor_frame, textvariable=rpm_var, width=12).grid(row=row, column=4, sticky="w", padx=6, pady=4)
            ttk.Button(
                self.motor_frame,
                text="Set",
                command=lambda motor_id=device_id: self.on_motor_set_clicked(motor_id),
            ).grid(row=row, column=5, sticky="w", padx=6, pady=4)
            ttk.Label(self.motor_frame, textvariable=set_rpm_var).grid(row=row, column=6, sticky="w", padx=6, pady=4)
            ttk.Label(self.motor_frame, textvariable=current_rpm_var).grid(row=row, column=7, sticky="w", padx=6, pady=4)
            ttk.Label(self.motor_frame, textvariable=torque_var).grid(row=row, column=8, sticky="w", padx=6, pady=4)
            ttk.Label(self.motor_frame, textvariable=command_var).grid(row=row, column=9, sticky="w", padx=6, pady=4)
    ...
    def refresh_current_motor_rpms(self):
        for cfg in MOTOR_CONFIGS:
            ...
            torque_var = getattr(self, 'motor_torque_vars', {}).get(device_id)
            detected_var = self.motor_detected_vars.get(device_id)

            if entry is None or current_var is None or torque_var is None or detected_var is None:
                continue

            detected = refresh_motor_detected(entry)
            detected_var.set("YES" if detected else "NO")
            ...
            # Estimate output torque from TalonFX torque-current signal.
            torque_val = None
            try:
                motor = entry.get("motor")
                if motor is not None and hasattr(motor, "get_torque_current"):
                    torque_current_signal = motor.get_torque_current()
                    torque_current_a = get_signal_double(torque_current_signal)
                    if torque_current_a is not None:
                        torque_val = torque_current_a * MOTOR_TORQUE_NM_PER_AMP
            except Exception:
                torque_val = None
            torque_var.set("---" if torque_val is None else f"{torque_val:.2f}")
    ...
    def update_motor_command_rows(self):
        for cfg in MOTOR_CONFIGS:
            ...
            if not ENABLE_MOTORS:
                ...
                command_var.set(
                    f"ON -> {rpm:.0f} RPM ({output:.3f}, {output * 100.0:.1f}%)"
                    if desired_on
                    else "OFF -> 0 RPM"
                )
    ...
    def collect_bottom_messages(self):
        ...
        for device_id in sorted(motors.keys()):
            entry = motors.get(device_id, {})
            desired_on = self.motor_enabled_vars.get(device_id)

            if desired_on is not None and desired_on.get() and not entry.get("detected", False):
                name = entry.get("name", f"Motor ID {device_id}")
                det_status = entry.get("detection_status", "UNKNOWN")
                messages.append(
                    f"{name}: not detected on CAN bus '{CAN_BUS_NAME}' ({det_status})"
                )

            if desired_on is not None and desired_on.get() and abs(entry.get("set_rpm", 0.0)) > 0:
                output = entry.get("last_output", 0.0)
                if abs(output) < 0.02:
                    name = entry.get("name", f"Motor ID {device_id}")
                    messages.append(
                        f"{name}: command is only {output * 100.0:.2f}% duty. Try a higher target RPM."
                    )
